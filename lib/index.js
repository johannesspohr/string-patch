...
exports.applyPatchOnPatch = applyPatchOnPatch;
exports.default = smartMerge;
var OPERATION_REMOVE = 'rm';
var OPERATION_INSERT = 'ins';
var OPERATION_REPLACE = 'rep';
...
function applyPatch(string, patch) {
    var res = string;
...
function applyPatchOnPatch(patchedPatch, patch) {
    for (var i in patch) {
        if (patch[i].operation != OPERATION_REPLACE) for (var j in patchedPatch) {
            if (patch[i].operation == OPERATION_INSERT && patchedPatch[j].position >= patch[i].position) patchedPatch[j].position += 1;
    ...
    return patchedPatch.filter(function (p) {
        return !p.removed;
    });
}

function smartMerge(fromString, toString, overwritingString, transformPositions) {
    ...
    if (transformPositions !== undefined) {
        for (var i = 0; i < patch1.length; i++) {
            for (var j = 0; j < transformPositions.length; j++) {
                if (patch1[i].operation == OPERATION_INSERT && transformPositions[j] >= patch1[i].position) transformPositions[j]++;else if (patch1[i].operation == OPERATION_REMOVE && transformPositions[j] >= patch1[i].position) transformPositions[j]--;
            }
        }
    }
