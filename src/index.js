...
export function createPatch(from, to) {
    ...
    for (var rowIndex = 0; rowIndex < toString.length; rowIndex++) {
        ...
        for (var colIndex = 0; colIndex < fromString.length; colIndex++) {
            ...
            if (rowIndex > 0)
                insertDistance = lastRow[colIndex].distance + 1;
            if (colIndex > 0)
                removeDistance = row[colIndex - 1].distance + 1;
            if (rowIndex > 0 && colIndex > 0)
                replaceDistance = lastRow[colIndex - 1].distance + (charsAreEqual ? 0 : 1);
...
export function applyPatch(string, patch) {
    ...
    for(var i = 0; i < patch.length; i++) {
        ...
        if (patch[i].operation != OPERATION_REMOVE)
            prefix += patch[i].char;
        res = prefix + postfix;
    }
    return res;
    ...
    if (transformPositions !== undefined) {
        for (var i = 0; i < patch1.length; i++) {
            for (var j = 0; j < transformPositions.length; j++) {
                if (patch1[i].operation == OPERATION_INSERT && transformPositions[j] >= patch1[i].position)
